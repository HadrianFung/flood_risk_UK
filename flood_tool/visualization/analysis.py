...
def plot_risk_map(risk_data, coordinate=["easting", "northing"], dx=1000):
    ...
    # process risk_data in the format we can manipulate
    risk_data = pd.DataFrame({
        'easting': [idx[0] for idx in risk_data.index],
        'northing': [idx[1] for idx in risk_data.index],
        'risk': risk_data.values
    })
    ...
    for index, row in risk_data.iterrows():
        Z[math.floor((row['easting'] - bbox[0]) / dx), math.floor((row['northing'] - bbox[2]) / dx)] += row['risk']
